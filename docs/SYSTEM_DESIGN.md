...
### upload_failures 与 upload_receipts（第二阶段）

`upload_failures` 保存传输或格式校验失败，不混入 OCR 任务状态。失败列表分页
查询，重试通过失败 ID 和批次内 client_id 精确关联。`upload_receipts` 保存
服务端已经确认的批次、client_id 与 evidence，用作浏览器失败元数据补写的
幂等栅栏；成功确认、批次图片关联和失败解决在同一事务完成。
上传字节先写入 `.incoming` 并校验；正式内容寻址文件只在上述事务取得写锁、
再次验证回执后落盘。正常并发冲突或事务回滚不会留下未引用的正式文件。
