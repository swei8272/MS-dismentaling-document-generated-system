...
每次领取返回的 `attempt_count` 是该次领取的版本。三个终态接口均必须接收
关键字参数 `attempt_count`，并在同一条 UPDATE 中校验任务 ID、processing
状态及版本。超时恢复不得重置计数；重新领取递增版本，拒绝旧 Worker 的
completed、pending 或 failed 通知。调用方须保留领取时的版本，不能提交前
重新查询当前版本。旧调用方必须同步更新，不提供绕过校验的默认参数。
本规则使用已有字段，无需新增数据库迁移。
