...
- 上传失败分页查询、逐条选择替换文件和向已有批次继续补充图片；
- 上传结束及状态轮询后局部更新当前页图片表格和分页；翻页不清空待上传文件，更新失败时保留旧列表并提示重试；
- 断网时只在浏览器保存必要的失败元数据，联网后有限重试同步；上传回执阻止迟到补写重开已解决失败。
- 图片先在 `.incoming` 校验和计算 SHA-256；精确回执复核通过后才在数据库写事务内落到内容寻址路径，失败事务清理本请求新建的文件。
...
默认数据库和图片目录从项目文件位置推导，分别为 `data\vehicles.db` 和 `data\uploads`。程序只会通过版本化迁移升级数据库，不会重建已有数据库。第二阶段初始迁移 2 保持不变；审查修复通过迁移 3 增加上传回执和失败解决证据引用。若检测到无法识别的旧 `evidence` 表结构，程序会在写入迁移前停止并报告。

迁移 3 不猜测升级前已经解决的失败记录对应哪张图片；这类记录会保留，并拒绝无法验证内容身份的重复请求。
